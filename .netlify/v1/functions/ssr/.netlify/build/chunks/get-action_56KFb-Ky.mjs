import { A as AstroError, q as ActionNotFoundError } from './astro/assets-service_CkejHdcM.mjs';

async function getAction(path) {
  const pathKeys = path.replace(/^.*\/_actions\//, "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_C-MelTC9.mjs');
